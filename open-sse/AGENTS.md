...
- **Commit messages**: never put `org/repo#NNNN` / `#NNNN` references in a commit message that will be pushed — GitHub auto-logs a permanent, non-deletable cross-reference event on the PR/issue, and amend/force-push does **not** remove it. Always ask the user before including any such reference; default to omitting it (put references in the PR description instead). Full SOP in `../CLAUDE.md` → "Commit-message cross-references".
