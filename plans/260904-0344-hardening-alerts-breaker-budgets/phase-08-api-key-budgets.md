...
1. **[SCOUT — RESOLVED; quick verify only, not a gate]** Trace chat usage recording — chain verified (Key Insight 1): apiKey IS stamped on all usage paths (chat.js:312 → chatCore.js:62/:471 → saveUsageStats at streamingHandler.js:139 / sseToJsonHandler.js:207+307 / nonStreamingHandler.js:321 → requestDetail.js:118 → usageRepo.js:301/:317; embeddings.js:140). At phase start, re-verify the listed lines still hold (grep); no gap list or user decision needed — failed requests (0 tokens) dropped at requestDetail.js:103 is acceptable.
